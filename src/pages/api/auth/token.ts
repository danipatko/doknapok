 ...
 */
export const getID = (req: NextApiRequest, res: NextApiResponse, admin: 'admin' | 'user' | 'any' = 'user'): string | undefined => {
    if (!process.env.JWT_SECRET) return;
    const token = getCookie('token', { req, res }) as string | undefined;
    if (!token) return;

    const payload = jwt.verify(token, process.env.JWT_SECRET) as jwt.JwtPayload;
    return admin == 'user' ? payload.id : admin == 'admin' ? payload.ad : payload.id ?? payload.ad;
};
 ...
 */
export const getUser = async (req: NextApiRequest, res: NextApiResponse, admin: 'admin' | 'user' | 'any' = 'user'): Promise<UserEntity | null> => {
